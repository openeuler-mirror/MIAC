...
TEST_F(ArithmeticOptimizerTest,
  ...
  auto expected =
      EvaluateNodes(item.graph, item.fetch, {{"a", a_t}, {"b", b_t}});
  ASSERT_EQ(expected.size(), 1);
  ...
  EXPECT_EQ(CountOpNodes(g, "Pack"), 1);

  NodeMap node_map(&g);
  const NodeDef* output_node = node_map.GetNode("output");
  ASSERT_NE(output_node, nullptr);
  ASSERT_EQ(output_node->input_size(), 1);
  const NodeDef* rewritten_pack =
      node_map.GetNode(NodeName(output_node->input(0)));
  ASSERT_NE(rewritten_pack, nullptr);
  EXPECT_EQ(rewritten_pack->op(), "Pack");
  EXPECT_EQ(rewritten_pack->input_size(), 1);
  EXPECT_EQ(rewritten_pack->input(0), "b");

  auto result = EvaluateNodes(g, item.fetch, {{"b", b_t}});
  ASSERT_EQ(result.size(), 1);
  test::ExpectTensorNear<float>(result[0], expected[0], 1e-6);
}
